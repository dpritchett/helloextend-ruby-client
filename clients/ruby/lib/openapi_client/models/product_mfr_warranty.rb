...
module OpenapiClient
  ...
  class ProductMfrWarranty
    # Length of the manufacturers warranty for parts in months.
    attr_accessor :parts

    # Length of the manufacturers warranty for labor in months.
    attr_accessor :labor

    # Link to the manufacturers warranty for this product.
    attr_accessor :url
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ProductMfrWarranty` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ProductMfrWarranty`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'labor')
        self.labor = attributes[:'labor']
      end
