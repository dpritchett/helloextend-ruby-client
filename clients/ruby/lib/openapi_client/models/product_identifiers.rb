...
module OpenapiClient
  class ProductIdentifiers
    # A unique identifier for the product that is specific to your store
    attr_accessor :sku

    # <p>Global Trade Item Number (GTIN) - A unique identifier for trade items, developed by GS1. <a href='https://www.gs1.org/standards/id-keys/gtin'>Learn more here</a></p>
    attr_accessor :gtin

    # The Universal Product Code (UPC) barcode
    attr_accessor :upc

    # The Amazon Standard Identification Number (ASIN)
    attr_accessor :asin

    # The product's barcode. Non-Standard
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ProductIdentifiers` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ProductIdentifiers`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [sku, gtin, upc, asin, barcode].hash
    end
