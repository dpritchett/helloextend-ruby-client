...
module OpenapiClient
  class InvoiceMetadata
    # An optional purchase Order number referenced in the original contract
    attr_accessor :order_reference_id

    # The plan that the contract applied to
    ...
    def self.openapi_types
      {
        ...
        :'plan_id' => :'String',
        :'product' => :'InvoiceMetadataProduct'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::InvoiceMetadata` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::InvoiceMetadata`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [order_reference_id, plan_id, product].hash
    end
